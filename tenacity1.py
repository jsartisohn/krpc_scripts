  ...
  try:
    ...
    while connection:
      ...
      if mag(ship.surface_velocity) < 60:
        ...
        last = align(ship, heading2vector(ship, pitch - 5, yaw), last=last)
      elif first_burn_start < 0 and mission_time > 8*60:
        if ship.latitude < 3 and ship.latitude > 1:
          last['Tf'] = 0.5
          set_rcs(ship, True)
          set_throttle(ship, 0)
          proceed_to_stage(ship, 2)
        elif mag(last['error']) < 0.01 and ship.latitude < 1:
          last['Tf'] = 1.5
          set_rcs(ship, False)
          set_throttle(ship, 1)
          proceed_to_stage(ship, 1)
          first_burn_start = time.time()
        last = align(ship, heading2vector(ship, 0, yaw), last=last)
      elif first_burn_start > 0 and first_burn_end < 0:
        if ship.orbit.apoapsis_altitude < 17204101:
          last = align(ship, heading2vector(ship, 0, yaw), last=last)
        else:
          set_throttle(ship, 0)
        ...
        if ship.orbit.periapsis_altitude < 16355689:
          last = align(ship, ship.orbit_velocity, last=last)
          first_burn_duration = first_burn_end - first_burn_start
          if time.time() - second_burn_start + first_burn_duration > 50:
